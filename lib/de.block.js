...
de.Block.prototype._init = function(options) {
    ...
    //      }
    //
    // FIXME: точно нужна проверка, или достаточно написать в документации
    // про допустимые форматы?
    if (typeof options.state === 'object' || typeof options.state === 'function') {
        _options.state = compileExpr(options.state);
    }
...
de.Block.prototype.log_end = function(context, msg, t1) {
    context.log_end('debug', this.debug_id() + ' ' + msg, t1 );
};

de.Block.prototype.log_error = function(context, msg) {
    context.error('error', this.debug_id() + ' ' + msg);
};

de.Block.prototype.debug_id = function() {
    return '[block.' + this._id + ' ' + JSON.stringify(this._block) + ']';
};
    ...
    switch (typeof block) {
        ...
        case 'string':
            ...
            //  который возвращает уникальную строку вида '@block25@'.
                ...
                //  FIXME после фикса include блока в #96 мы делаем eval один раз, а compile - несколько раз.
                //  Поэтому нам нужно, чтобы eval-енные блоки оставались в _blocks.
                //  delete _blocks[id];
            }
...
de.Block.Http = function(url, options) {
    ...
    var _options = this._init(options);

    //  Пробрасывать ли http-заголовки.
    //  По дефолту -- пробрасывать.
    _options.proxy = (options.proxy === undefined) ? true : options.proxy;

    _options.method = ( options.method || 'get' ).toLowerCase();
...
de.Block.Call.prototype._run = function(params, context) {
    ...
    if (module) {
        ...
        try {
            ...
            this.log_error(context, `[caught error: ${error.stack}]`);
        ...
        this.log_error(context, `[module not found: ${this.moduleName}]`);
        return no.Promise.resolved( de.error({
            id: 'MODULE_NOT_FOUND',
...
de.Block.Function.prototype.log_end = no.nop;
    ...
    if (!including) {
        ...
        de.file.eval(filename, 'de', de.sandbox, context)
            .done(function(include) {
                context.log_end('debug', '[include.eval ' + JSON.stringify(filename) + '] ended', t1);
                including.resolve(include);
            })
            .fail(function(error) {
                context.log_end('info', '[include.eval ' + JSON.stringify(filename) + '] failed', t1);
...
//
de.events.on('loaded-file-changed', function(e, filename) {
    _includes[filename] = null;
});
...
de.Block.Value.prototype.log_end = no.nop;
...
de.Block.Array.prototype.log_end = no.nop;
...
de.Block.Object.prototype.log_end = no.nop;
...
//  FIXME: А это вообще где-нибудь используется?
...
de.Block.Expr.prototype.log_end = no.nop;
